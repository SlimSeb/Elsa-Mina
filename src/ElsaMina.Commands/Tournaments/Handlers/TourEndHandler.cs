...
public class TourEndHandler : Handler
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            AnnounceWinnerPrize(result, room, roomId);
        }

        await DisplayWinnerProfileAsync(result.Winner, roomId, cancellationToken);
    }

    private async Task SaveTournamentResultsAsync(TournamentResults result, string roomId, bool isBucksEnabled,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            await SaveTournamentRecordsAsync(dbContext, result, roomId, cancellationToken);
            await AwardWinnerPrizeAsync(dbContext, result, roomId, isBucksEnabled, cancellationToken);
    ...
    private async Task SaveTournamentRecordsAsync(BotDbContext dbContext, TournamentResults result, string roomId,
        CancellationToken cancellationToken)
    {
    ...
    private static async Task AwardWinnerPrizeAsync(BotDbContext dbContext, TournamentResults result, string roomId,
        bool isBucksEnabled, CancellationToken cancellationToken)
    {
        ...
        // The winner is one of the players, so their room data was created while saving the records.
    ...
    private void AnnounceWinnerPrize(TournamentResults result, IRoom room, string roomId)
    {
        var prize = result.Players.Count * PRIZE_PER_PARTICIPANT;
    ...
    private async Task DisplayWinnerProfileAsync(string winner, string roomId, CancellationToken cancellationToken)
    {
        try
        {
            var profileHtml = await _profileService.GetProfileHtmlAsync(winner, roomId, cancellationToken);
            _bot.Say(roomId, $"/addhtmlbox {profileHtml.RemoveNewlines()}");
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error displaying winner profile for {Winner} in room {RoomId}", winner, roomId);
        }
    }
}
