...
public class RandPastaCommand : Command
{
    private readonly ISpoonacularService _spoonacularService;

    public RandPastaCommand(ISpoonacularService spoonacularService)
    {
    ...
    public override async Task RunAsync(IContext context, CancellationToken cancellationToken = default)
    {
        try
        {
            var html = await _spoonacularService.GetRandomRecipeHtmlAsync("pasta", cancellationToken);
            if (html == null)
            {
        ...
        {
            Log.Error(ex, "Failed to fetch random pasta recipe.");
