// <auto-generated />
using System;
using ElsaMina.DataAccess;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;

#nullable disable

namespace ElsaMina.DataAccess.Migrations
{
    [DbContext(typeof(BotDbContext))]
    [Migration("20260923145444_AddChannelAlertsTable")]
    partial class AddChannelAlertsTable
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.8")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("ElsaMina.DataAccess.Models.AddedCommand", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("Author")
                        .HasColumnType("text");

                    b.Property<string>("Content")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreationDate")
                        .HasColumnType("timestamp with time zone");

                    b.HasKey("Id", "RoomId");

                    b.HasIndex("RoomId");

                    b.ToTable("AddedCommands");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ArcadeLevel", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<int>("Level")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.ToTable("ArcadeLevels");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.Badge", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("Image")
                        .HasColumnType("text");

                    b.Property<bool>("IsTeamTournament")
                        .HasColumnType("boolean");

                    b.Property<bool>("IsTrophy")
                        .HasColumnType("boolean");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.HasKey("Id", "RoomId");

                    b.HasIndex("RoomId");

                    b.ToTable("Badges");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BadgeHolding", b =>
                {
                    b.Property<string>("BadgeId")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.HasKey("BadgeId", "UserId", "RoomId");

                    b.HasIndex("BadgeId", "RoomId");

                    b.HasIndex("UserId", "RoomId");

                    b.ToTable("BadgeHoldings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BattleshipRating", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("Losses")
                        .HasColumnType("integer");

                    b.Property<int>("Rating")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("BattleshipRatings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BeloteStats", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("GamesPlayed")
                        .HasColumnType("integer");

                    b.Property<int>("TakerWins")
                        .HasColumnType("integer");

                    b.Property<int>("TimesTaker")
                        .HasColumnType("integer");

                    b.Property<int>("TotalScore")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("BeloteStats");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BetRecord", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<int>("CorrectBetsCount")
                        .HasColumnType("integer");

                    b.Property<int>("TotalBetsCount")
                        .HasColumnType("integer");

                    b.HasKey("UserId", "RoomId");

                    b.ToTable("BetRecords");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ChannelAlert", b =>
                {
                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("Platform")
                        .HasColumnType("text");

                    b.Property<string>("ChannelId")
                        .HasColumnType("text");

                    b.Property<string>("ChannelName")
                        .HasColumnType("text");

                    b.HasKey("RoomId", "Platform", "ChannelId");

                    b.ToTable("ChannelAlerts");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ChessRating", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("Draws")
                        .HasColumnType("integer");

                    b.Property<int>("Losses")
                        .HasColumnType("integer");

                    b.Property<int>("Rating")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("ChessRatings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ConnectFourRating", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("Draws")
                        .HasColumnType("integer");

                    b.Property<int>("Losses")
                        .HasColumnType("integer");

                    b.Property<int>("Rating")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("ConnectFourRatings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.DollHolding", b =>
                {
                    b.Property<string>("DollId")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<int>("Position")
                        .HasColumnType("integer");

                    b.HasKey("DollId", "UserId", "RoomId");

                    b.HasIndex("UserId", "RoomId");

                    b.ToTable("DollHoldings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.EventRoleMapping", b =>
                {
                    b.Property<string>("EventName")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("DiscordRoleId")
                        .HasColumnType("text");

                    b.HasKey("EventName", "RoomId");

                    b.ToTable("EventRoleMappings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.FloodItScore", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("BestMoves")
                        .HasColumnType("integer");

                    b.Property<int>("Level")
                        .HasColumnType("integer");

                    b.Property<int>("TotalStars")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("FloodItScores");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.LadderEloSnapshot", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<int>("Elo")
                        .HasColumnType("integer");

                    b.Property<string>("Format")
                        .HasColumnType("text");

                    b.Property<DateTime>("RecordedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RecordedAt");

                    b.HasIndex("UserId", "Format");

                    b.ToTable("LadderEloSnapshots");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.LightsOutScore", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("BestMoves")
                        .HasColumnType("integer");

                    b.Property<int>("Level")
                        .HasColumnType("integer");

                    b.Property<int>("TotalStars")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("LightsOutScores");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.NameColor", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("Color")
                        .HasColumnType("text");

                    b.HasKey("UserId");

                    b.ToTable("NameColors");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.PollSuggestion", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Content")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("UserName")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("PollSuggestions");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.PollSuggestionBan", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.HasKey("UserId", "RoomId");

                    b.ToTable("PollSuggestionBans");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomBotParameterValue", b =>
                {
                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("ParameterId")
                        .HasColumnType("text");

                    b.Property<string>("Value")
                        .HasColumnType("text");

                    b.HasKey("RoomId", "ParameterId");

                    b.ToTable("RoomBotParameterValues");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomTeam", b =>
                {
                    b.Property<string>("TeamId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.HasKey("TeamId", "RoomId");

                    b.HasIndex("RoomId");

                    b.ToTable("RoomTeams");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("Avatar")
                        .HasColumnType("text");

                    b.Property<int>("CurrentStreak")
                        .HasColumnType("integer");

                    b.Property<string>("JoinPhrase")
                        .HasColumnType("text");

                    b.Property<DateOnly?>("LastActivityDate")
                        .HasColumnType("date");

                    b.Property<int>("LongestStreak")
                        .HasColumnType("integer");

                    b.Property<long>("Money")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bigint")
                        .HasDefaultValue(100L);

                    b.Property<TimeSpan>("PlayTime")
                        .HasColumnType("interval");

                    b.Property<string>("ProfileBackgroundColor")
                        .HasColumnType("text");

                    b.Property<string>("ProfileEmoji")
                        .HasColumnType("text");

                    b.Property<string>("ProfileLabelColor")
                        .HasColumnType("text");

                    b.Property<string>("ProfileTextColor")
                        .HasColumnType("text");

                    b.Property<string>("Title")
                        .HasColumnType("text");

                    b.HasKey("Id", "RoomId");

                    b.ToTable("RoomUsers");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedPoll", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Content")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset>("EndedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RoomId");

                    b.ToTable("SavedPolls");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedRepeat", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<TimeSpan>("Interval")
                        .HasColumnType("interval");

                    b.Property<string>("Message")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Repeats");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedRoom", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("Title")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Rooms");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedTournament", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<DateTimeOffset>("EndedAt")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Format")
                        .HasColumnType("text");

                    b.Property<int>("PlayerCount")
                        .HasColumnType("integer");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("RunnerUp")
                        .HasColumnType("text");

                    b.Property<string>("SemiFinalists")
                        .HasColumnType("text");

                    b.Property<string>("Winner")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.HasIndex("RoomId");

                    b.ToTable("SavedTournaments");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedUser", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset?>("LastOnline")
                        .HasColumnType("timestamp with time zone");

                    b.Property<int>("LastSeenAction")
                        .HasColumnType("integer");

                    b.Property<string>("LastSeenRoomId")
                        .HasColumnType("text");

                    b.Property<DateTimeOffset?>("RegisterDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("UserName")
                        .HasColumnType("text");

                    b.HasKey("UserId");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SemantixScore", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("BestGuessCount")
                        .HasColumnType("integer");

                    b.Property<int>("CurrentStreak")
                        .HasColumnType("integer");

                    b.Property<int>("GamesPlayed")
                        .HasColumnType("integer");

                    b.Property<DateOnly?>("LastWonDate")
                        .HasColumnType("date");

                    b.Property<int>("MaxStreak")
                        .HasColumnType("integer");

                    b.Property<int>("TotalGuesses")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("SemantixScores");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ShopItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("integer");

                    NpgsqlPropertyBuilderExtensions.UseIdentityByDefaultColumn(b.Property<int>("Id"));

                    b.Property<string>("Article")
                        .HasColumnType("text");

                    b.Property<string>("Price")
                        .HasColumnType("text");

                    b.Property<string>("Tier")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("ShopItems");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.StaffIntro", b =>
                {
                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("HtmlContent")
                        .HasColumnType("text");

                    b.HasKey("RoomId");

                    b.ToTable("StaffIntros");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TarotStats", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("GamesPlayed")
                        .HasColumnType("integer");

                    b.Property<int>("TakerWins")
                        .HasColumnType("integer");

                    b.Property<int>("TimesTaker")
                        .HasColumnType("integer");

                    b.Property<int>("TotalScoreHalfPoints")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("TarotStats");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.Team", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("Author")
                        .HasColumnType("text");

                    b.Property<DateTime>("CreationDate")
                        .HasColumnType("timestamp with time zone");

                    b.Property<string>("Format")
                        .HasColumnType("text");

                    b.Property<string>("Link")
                        .HasColumnType("text");

                    b.Property<string>("Name")
                        .HasColumnType("text");

                    b.Property<string>("TeamJson")
                        .HasColumnType("text");

                    b.HasKey("Id");

                    b.ToTable("Teams");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TourConfig", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<int?>("AutoDq")
                        .HasColumnType("integer");

                    b.Property<int>("Autostart")
                        .HasColumnType("integer");

                    b.Property<string>("Format")
                        .HasColumnType("text");

                    b.Property<string>("Rules")
                        .HasColumnType("text");

                    b.Property<string>("Teams")
                        .HasColumnType("text");

                    b.Property<string>("Tier")
                        .HasColumnType("text");

                    b.Property<string>("TourName")
                        .HasColumnType("text");

                    b.HasKey("Id", "RoomId");

                    b.ToTable("TourConfigs");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TournamentRecord", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<int>("PlayedGames")
                        .HasColumnType("integer");

                    b.Property<int>("RunnerUpCount")
                        .HasColumnType("integer");

                    b.Property<int>("ThirdPlaceCount")
                        .HasColumnType("integer");

                    b.Property<int>("TournamentsEnteredCount")
                        .HasColumnType("integer");

                    b.Property<int>("WinsCount")
                        .HasColumnType("integer");

                    b.Property<int>("WonGames")
                        .HasColumnType("integer");

                    b.HasKey("UserId", "RoomId");

                    b.ToTable("TournamentRecords");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TrackedEloUser", b =>
                {
                    b.Property<string>("Format")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.HasKey("Format", "UserId");

                    b.HasIndex("UserId");

                    b.ToTable("TrackedEloUsers");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TwentyFortyEightScore", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("BestScore")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("TwentyFortyEightScores");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.UserPoints", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("text");

                    b.Property<double>("Points")
                        .HasColumnType("double precision");

                    b.HasKey("Id");

                    b.ToTable("UserPoints");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.VoltorbFlipLevel", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("Coins")
                        .HasColumnType("integer");

                    b.Property<int>("Level")
                        .HasColumnType("integer");

                    b.Property<int>("MaxLevel")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("VoltorbFlipLevels");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.WatchlistEntry", b =>
                {
                    b.Property<string>("RoomId")
                        .HasColumnType("text");

                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<string>("Rank")
                        .HasColumnType("text");

                    b.HasKey("RoomId", "UserId");

                    b.ToTable("WatchlistEntries");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.WordEmbedding", b =>
                {
                    b.Property<string>("Word")
                        .HasColumnType("text");

                    b.Property<string>("Model")
                        .HasColumnType("text");

                    b.Property<byte[]>("Vector")
                        .HasColumnType("bytea");

                    b.HasKey("Word", "Model");

                    b.ToTable("WordEmbeddings");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.WordleScore", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("text");

                    b.Property<int>("CurrentStreak")
                        .HasColumnType("integer");

                    b.Property<int>("GamesPlayed")
                        .HasColumnType("integer");

                    b.Property<DateOnly?>("LastPlayedDate")
                        .HasColumnType("date");

                    b.Property<int>("MaxStreak")
                        .HasColumnType("integer");

                    b.Property<int>("TotalGuesses")
                        .HasColumnType("integer");

                    b.Property<int>("Wins")
                        .HasColumnType("integer");

                    b.HasKey("UserId");

                    b.ToTable("WordleScores");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.AddedCommand", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", "SavedRoom")
                        .WithMany("AddedCommands")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SavedRoom");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ArcadeLevel", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", null)
                        .WithMany()
                        .HasForeignKey("Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.Badge", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", null)
                        .WithMany("Badges")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BadgeHolding", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.Badge", "Badge")
                        .WithMany("BadgeHolders")
                        .HasForeignKey("BadgeId", "RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ElsaMina.DataAccess.Models.RoomUser", "RoomUser")
                        .WithMany("Badges")
                        .HasForeignKey("UserId", "RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Badge");

                    b.Navigation("RoomUser");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BattleshipRating", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("BattleshipRating")
                        .HasForeignKey("ElsaMina.DataAccess.Models.BattleshipRating", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BeloteStats", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("BeloteStats")
                        .HasForeignKey("ElsaMina.DataAccess.Models.BeloteStats", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.BetRecord", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.RoomUser", "RoomUser")
                        .WithOne("BetRecord")
                        .HasForeignKey("ElsaMina.DataAccess.Models.BetRecord", "UserId", "RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RoomUser");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ChessRating", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("ChessRating")
                        .HasForeignKey("ElsaMina.DataAccess.Models.ChessRating", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.ConnectFourRating", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("ConnectFourRating")
                        .HasForeignKey("ElsaMina.DataAccess.Models.ConnectFourRating", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.DollHolding", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.RoomUser", "RoomUser")
                        .WithMany("Dolls")
                        .HasForeignKey("UserId", "RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RoomUser");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.FloodItScore", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("FloodItScore")
                        .HasForeignKey("ElsaMina.DataAccess.Models.FloodItScore", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.LadderEloSnapshot", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade);
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.LightsOutScore", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("LightsOutScore")
                        .HasForeignKey("ElsaMina.DataAccess.Models.LightsOutScore", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.NameColor", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomBotParameterValue", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", "SavedRoom")
                        .WithMany("ParameterValues")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SavedRoom");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomTeam", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", "SavedRoom")
                        .WithMany("Teams")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("ElsaMina.DataAccess.Models.Team", "Team")
                        .WithMany("Rooms")
                        .HasForeignKey("TeamId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SavedRoom");

                    b.Navigation("Team");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomUser", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithMany("RoomData")
                        .HasForeignKey("Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedPoll", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", "SavedRoom")
                        .WithMany("PollHistory")
                        .HasForeignKey("RoomId");

                    b.Navigation("SavedRoom");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedTournament", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedRoom", "SavedRoom")
                        .WithMany("TournamentHistory")
                        .HasForeignKey("RoomId");

                    b.Navigation("SavedRoom");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SemantixScore", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("SemantixScore")
                        .HasForeignKey("ElsaMina.DataAccess.Models.SemantixScore", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TarotStats", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("TarotStats")
                        .HasForeignKey("ElsaMina.DataAccess.Models.TarotStats", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TournamentRecord", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.RoomUser", "RoomUser")
                        .WithOne("TournamentRecord")
                        .HasForeignKey("ElsaMina.DataAccess.Models.TournamentRecord", "UserId", "RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("RoomUser");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TrackedEloUser", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.TwentyFortyEightScore", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("TwentyFortyEightScore")
                        .HasForeignKey("ElsaMina.DataAccess.Models.TwentyFortyEightScore", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.UserPoints", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", null)
                        .WithMany()
                        .HasForeignKey("Id")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.VoltorbFlipLevel", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("VoltorbFlipLevel")
                        .HasForeignKey("ElsaMina.DataAccess.Models.VoltorbFlipLevel", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.WordleScore", b =>
                {
                    b.HasOne("ElsaMina.DataAccess.Models.SavedUser", "User")
                        .WithOne("WordleScore")
                        .HasForeignKey("ElsaMina.DataAccess.Models.WordleScore", "UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("User");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.Badge", b =>
                {
                    b.Navigation("BadgeHolders");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.RoomUser", b =>
                {
                    b.Navigation("Badges");

                    b.Navigation("BetRecord");

                    b.Navigation("Dolls");

                    b.Navigation("TournamentRecord");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedRoom", b =>
                {
                    b.Navigation("AddedCommands");

                    b.Navigation("Badges");

                    b.Navigation("ParameterValues");

                    b.Navigation("PollHistory");

                    b.Navigation("Teams");

                    b.Navigation("TournamentHistory");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.SavedUser", b =>
                {
                    b.Navigation("BattleshipRating");

                    b.Navigation("BeloteStats");

                    b.Navigation("ChessRating");

                    b.Navigation("ConnectFourRating");

                    b.Navigation("FloodItScore");

                    b.Navigation("LightsOutScore");

                    b.Navigation("RoomData");

                    b.Navigation("SemantixScore");

                    b.Navigation("TarotStats");

                    b.Navigation("TwentyFortyEightScore");

                    b.Navigation("VoltorbFlipLevel");

                    b.Navigation("WordleScore");
                });

            modelBuilder.Entity("ElsaMina.DataAccess.Models.Team", b =>
                {
                    b.Navigation("Rooms");
                });
#pragma warning restore 612, 618
        }
    }
}
