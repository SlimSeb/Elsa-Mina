...
[TestFixture]
public class GifCooldownServiceTest
{
    private GifCooldownService _service;

    [SetUp]
    public void SetUp()
    {
        _service = new GifCooldownService();
    }
    ...
    public void Test_GetRemainingCooldowns_ShouldReturnZeroForRoom_WhenRoomCooldownHasExpired()
    {
        ...
        var (roomRemaining, _) =
            _service.GetRemainingCooldowns("room1", "user2", now + GifConstants.PER_ROOM_COOLDOWN + TimeSpan.FromSeconds(1));
    ...
    public void Test_GetRemainingCooldowns_ShouldReturnUserRemaining_WhenUserIsOnCooldown()
    {
        ...
        Assert.That(userRemaining,
            Is.EqualTo(GifConstants.PER_USER_COOLDOWN - TimeSpan.FromMinutes(5))
                .Within(TimeSpan.FromMilliseconds(1)));
    }
    ...
    public void Test_GetRemainingCooldowns_ShouldReturnZeroForUser_WhenUserCooldownHasExpired()
    {
        ...
        var (_, userRemaining) =
            _service.GetRemainingCooldowns("room2", "user1", now + GifConstants.PER_USER_COOLDOWN + TimeSpan.FromSeconds(1));
