...
[TestFixture]
public class RoomOptionsCommandTest
{
    private IContext _context;
    private IRoomsManager _roomsManager;
    private IRoomDashboardService _roomDashboardService;
    private RoomOptionsCommand _command;
    ...
    public void SetUp()
    {
        ...
        _command = new RoomOptionsCommand(_roomsManager, _roomDashboardService);
    ...
    [Test]
    public async Task Test_RoomOptions_ShouldDeny_WhenUserHasInsufficientRank()
    {
        ...
        await _roomDashboardService.DidNotReceive()
            .SendOptionsPageAsync(Arg.Any<IContext>(), Arg.Any<string>(), Arg.Any<CancellationToken>());
    }

    [Test]
    public async Task Test_RoomOptions_ShouldRenderOptionsPage_WhenUserIsRoomStaff()
    {
        var room = Substitute.For<IRoom>();
        _context.Target.Returns("testroom");
        _roomsManager.GetRoom("testroom").Returns(room);
    ...
    public async Task Test_RunAsync_ShouldUseFallbackRoomId_WhenTargetIsEmpty()
    {
        ...
        await _roomDashboardService.Received(1)
            .SendOptionsPageAsync(_context, "defaultroom", Arg.Any<CancellationToken>());
    }
